...
export default async function WorkshopHistory() {
  ...
  if (payments.length === 0) {
    return <Empty label="No workshop bookings found" />
  }

  return (
    <div className="space-y-6 max-h-[calc(100vh-260px)] overflow-y-auto pr-2">
      {payments.map((p) => {
        const booking = p.workShopBooking!
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {formatISTDate(workshop.startAt)} •{" "}
                    {formatISTTime(workshop.startAt)} –{" "}
                    {formatISTTime(workshop.endAt)}
                  </p>
                </div>
