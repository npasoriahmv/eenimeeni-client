...
export default async function VenueHistory() {
  ...
  if (payments.length === 0) {
    return <Empty label="No venue bookings found" />
  }

  return (
    <div className="space-y-6 max-h-[calc(100vh-260px)] overflow-y-auto pr-2">
      {payments.map((p) => {
        const venue = p.venueBooking!
          ...
          >
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                  <h3 className="text-lg font-semibold">Venue Booking</h3>
                  <p className="text-sm text-muted-foreground">
                    {formatISTDate(venue.startTime)} •{" "}
                    {formatISTTime(venue.startTime)} –{" "}
                    {formatISTTime(venue.endTime)}
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                <Detail
                  label="Duration"
                  value={`${venue.durationHours} hours`}
                />

                <Detail
                  label="Headcount"
                  value={venue.headcount}
                />
