...
export async function POST(request: NextRequest) {
  try {
    ...
    // -------------------------------
    // HANDLE EENIMEENIMINYMOE BOOKING
    // -------------------------------
    if (payment.emmmPartyBookingId) {
      if (success) {
        const booking = await prisma.emmmPartyBooking.findUnique({
          where: { id: payment.emmmPartyBookingId },
          include: {
            payments: {
        ...
        await prisma.emmmPartyBooking.update({
          where: { id: booking?.id },
          data: {
            bookingId: `${Booking_Prefix.eenimeeniminymoe}${booking?.bookingNumber}`
          }
        })
