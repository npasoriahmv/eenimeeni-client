...
export default function SuccessVenueBooking({ payment }: any) {
  const booking = payment.venueBooking
    ...
    <div className="max-w-xl mx-auto mt-12 p-6 border rounded-xl shadow-sm bg-white">
      <div className="text-center">
        ...
        <div className="mt-6 space-y-3 text-left text-sm">
          ...
          <div><strong>Date:</strong> {formatISTDate(booking.startTime)}</div>
          <div>
            <strong>Time:</strong>{" "}
            {formatISTTime(booking.startTime)} - {formatISTTime(booking.endTime)}
          </div>
          <div><strong>Duration:</strong> {booking.durationHours} hours</div>
          <div><strong>Headcount:</strong> {booking.headcount}</div>
        </div>
